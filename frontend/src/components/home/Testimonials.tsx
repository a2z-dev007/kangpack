...
const Testimonials: React.FC = () => {
  ...
  const TestimonialCard = ({ testimonial }: { testimonial: Testimonial }) => (
    <motion.div
      className="flex-shrink-0 cursor-pointer w-[280px] sm:w-[320px] md:w-[420px] xl:w-[480px] 2xl:w-[550px] bg-white/[0.02] rounded-2xl p-6 md:p-8 xl:p-10 2xl:p-12 shadow-2xl border border-white/5 dark-glass-hover transition-all duration-300"
      onHoverStart={() => setIsPaused(true)}
      onHoverEnd={() => setIsPaused(false)}
      whileHover={{
        scale: 1.03,
        y: -8,
        boxShadow: "0 20px 40px rgba(0,0,0,0.5), 0 0 40px rgba(166,124,82,0.08)",
    ...
    >
      ...
      <div className="flex items-start justify-between mb-6 xl:mb-3">
        ...
        <div className="flex-shrink-0">
          <img
            ...
            className="w-14 h-14 xl:w-16 xl:h-16 rounded-full object-cover ring-2 ring-white/10"
          />
        </div>

        {/* X Icon */}
        <div className="w-5 h-5 text-white/30 hover:text-white/60 cursor-pointer transition-colors">
      ...
      <div className="flex gap-1 mb-6 xl:mb-3">
        {[...Array(testimonial.rating)].map((_, i) => (
          <Star
            key={i}
            className="w-5 h-5 xl:w-4 xl:h-4 fill-[#a67c52] text-[#a67c52]"
          />
        ))}
      </div>

      {/* Testimonial Text */}
      <p className="text-white/70 text-lg xl:text-xl leading-relaxed mb-6 xl:mb-8">
        {testimonial.text}
      </p>
      <div className="w-full h-[1px] bg-white/10 mb-4 xl:mb-3" />
      {/* Author Info */}
      <div>
        ...
        <p className="text-white/30 text-sm mt-1">{testimonial.company}</p>
      </div>
    </motion.div>
  );
